...
def main():
    ...
    # Initialise the factors 1...N to unity. We just need to use a distribution
    # from the exponential family for this problem, a Gaussian is appropriate.
    # This corresponds to:
    f_means = np.zeros(N)
    f_vars = np.repeat(np.infty, N)
    f_ss = np.ones(N)
    ...
    for iteration in range(max_i):
        ...
        for n in range(N):
            ...
            if est_var != cav_var:
                ...
                if abs(f_means[n] - fn_mean_new) > tol and \
                    abs(f_vars[n] - fn_var_new) > tol and \
                        abs(f_ss[n] - fn_ss_new) > tol:
                    converged = False
                else:
                    converged = True

                # Saved refined factor parameters
                f_means[n] = fn_mean_new
                f_vars[n] = fn_var_new
                f_ss[n] = fn_ss_new

            # Plot the factors
            if f_vars[n] != np.inf and config['interactive']:
                plot_factors(X, n, f_means[n], f_vars[n], f_ss[n], cav_mean, cav_var)
